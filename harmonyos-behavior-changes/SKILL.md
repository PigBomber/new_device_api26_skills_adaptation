...
description: >
  ...
  块 B 基于 25 项变更，用 grep 扫项目代码，命中的才适配——不允许没命中就预防性修改。
...
**入口文件**：[references/migration-guide.md](references/migration-guide.md)
...
升级到 API 26 后，共 25 项行为变化（`changelogs-for-all-apps-7001` + `changelogs-ux-7001`）。
...
每项变更的「grep 关键词」是 changelog 里「变更的接口/组件」字段列出的 API/组件名。
...
> 完整变更说明（含变更前/后对比、适配代码示例）在 `/doc/roadmap/changelogs-for-all-apps-7001.md` 和 `/doc/roadmap/changelogs-ux-7001.md`。
...
- [references/migration-guide.md](references/migration-guide.md) — V1→V2 迁移指南（20项对照+5步流程+踩坑记录）
