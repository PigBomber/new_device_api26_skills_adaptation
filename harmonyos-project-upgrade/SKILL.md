...
description: 鸿蒙工程/项目整体大版本升级总入口。当开发者要把**整个鸿蒙工程**升到更高的 SDK **大版本**时触发，例如「帮我把这个项目升级到 26.0.0」「升到 7.0」「升到 26」「项目从 5.0.5 升到 26.0.0」「升级到 API 26」「升到 6.1」「升到 6.0」「升到 6.1.0」「鸿蒙工程升级」「compatibleSdkVersion 升级」「targetSdkVersion 升级影响」「升级到新版 SDK 后编译报错」「整体升版本要改哪些东西」——核心特征是**改 build-profile.json5 的 compatibleSdkVersion/targetSdkVersion**。版本归一：用户说"升到 6.0/6.1"归一为 6.1.0（路径A）；说"升到 7.0/26/最新版"归一为 26.0.0（路径B，鸿蒙7.0=26.0.0）。**只支持两条大版本路径**：升到 6.1.0(API23) 或升到 26.0.0(API26)。本 skill 是路由入口，不存业务数据，识别用户当前在升级流程的哪个环节后读取对应 reference 处理。触发后第一件事是用 TodoWrite 创建内置 todo 清单。不包含具体的技术实现细节（在 references 中）。**不触发/不支持**：① 单个组件/库的替换或升级（如「升级成 hds 组件」「换成 xxx 组件」「引入新组件库」）——只动单个依赖不改工程 SDK 版本号；② 同大版本内的小版本递进（如 6.1.0→6.3.0、5.0.5→5.1.0）——提示用户本 skill 只支持大版本升级。
---
...
收到升级请求后，**先按下面的版本归一规则确定目标版本**，再判断是否在支持范围内。

### 版本号归一规则（用户口语 → 实际目标版本）

| 用户说的 | 归一后的目标 | 路径 | 说明 |
|---------|------------|------|------|
| 「升到 6.0」「升到 6.1」「6 系最新」 | **6.1.0** | 路径A | 6.x 大版本归一到 6.1.0 |
| 「升到 7.0」「升到 26」「升到最新版」 | **26.0.0** | 路径B | 鸿蒙 7.0 = 版本号 26.0.0（SemVer 改名） |
| 「升到 6.1.0」 | 6.1.0 | 路径A | 精确版本，直接用 |
| 「升到 26.0.0」 | 26.0.0 | 路径B | 精确版本，直接用 |

### 范围判定（只支持大版本升级）

确定目标版本后，**先检测当前基线**（读 build-profile.json5 的 compatibleSdkVersion）：

- **当前版本 < 目标版本的大版本**（如 5.x→6.1.0、6.x→26.0.0）→ 支持，走对应路径
- **当前版本 ≥ 目标版本**（如当前已是 6.1.0 还想升到 6.1.0）→ 提示"已是该版本，无需升级"，退出
- **小版本升级（同大版本内的小版本递进，如 6.1.0→6.3.0、5.0.5→5.1.0）** → **不支持**。提示用户："本 skill 只支持大版本升级（升到 6.1.0 或 26.0.0），不支持 6.1.0→6.3.0 这类同大版本内的小版本递进。"

### 两条路径对比

| 目标版本 | 路径 | 版本号格式 | 废弃API | 行为变化 | V2状态管理迁移 | SDK 要求 |
|---------|------|----------|---------|---------|--------------|---------|
...
> 两条路径的流程骨架相同（检测→配置→废弃API→行为变化→验证），差异在每个环节的数据/参数，以及路径B 多一个 V2 迁移环节。
...
| 用户说的话 | Read / 处理 |
|---------------------|------|
| "升到 26.0.0" / "升到 7.0" / "升到 26" / "升到最新版" | 目标归一为 26.0.0 → **路径B**：创建 todo 清单，从步骤0开始（先检查 SDK≥26） |
| "升到 6.1" / "升到 6.0" / "升到 6.1.0" / "6 系最新" | 目标归一为 6.1.0 → **路径A**：创建 todo 清单，从步骤0开始（先检查 SDK≥23），跳过步骤4 V2迁移 |
| "升到 6.3" / "升到 5.1" / "6.1.0 升到 6.x" | **小版本升级不支持**：提示用户本 skill 只支持大版本升级（6.1.0 或 26.0.0），不支持同大版本内的小版本递进 |
| 当前版本已是目标版本（如已是 6.1.0 还要升 6.1.0） | 提示"已是该版本，无需升级"，退出 |
