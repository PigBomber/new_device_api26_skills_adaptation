...
> ⚠️ **仅路径B（→26.0.0）需要 V2 迁移**。路径A（→6.1.0）不涉及——V1 装饰器在 6.1.0 正常运行，本文件不适用。

> 本文件为 `SKILL.md` 的补充，记录 V1→V2 装饰器对照表、机制差异、5 步迁移步骤、每条规则的代码对比与迁移易错点。
...
|------|------|
| 新开发的应用 | 直接用 V2 |
| 老应用升级到 26.0.0（**路径B**，→API 26） | **必须迁移**。V1→V2 是路径B升级流程的必要环节，不留 V1 残留 |
| 老应用升级到 6.1.0（**路径A**，→API 23） | 不涉及 V2 迁移，V1 装饰器在 6.1.0 正常运行 |
