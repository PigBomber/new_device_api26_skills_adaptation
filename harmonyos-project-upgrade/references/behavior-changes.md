...
diagnostic_checklist:
  ...
  - For Block B: Has grep been run for each change item's keywords in the version-appropriate tables below?
...
本文件覆盖升级流程的两个环节：**块 A**（步骤4）状态管理 V1→V2 迁移——升级时必须迁移，不是可选项；**块 B**（步骤5）接口行为变化适配——基于多张版本累积速查表（路径A 约30项、路径B 约54项），用 grep 扫项目代码，命中的才适配。
...
| 怎么发现 | 查装饰器对照表；V1 装饰器 grep 统计 | 用各版本速查表的 grep 关键词扫项目代码 |
...
| `delete\s+\w+\[` ⚠️**grep 不精确** | 字面量定义的数组 delete 元素后再次使用该字面量，新数组由「保留删除后状态」变为「字面量原始值」 | grep `delete\s+\w+\[` 只能初步定位，需人工确认是否「字面量数组 + delete 后复用」；排查三方库/js 文件 |
...
| `@State.*Function\|@Prop.*Function` 等 ⚠️**grep 不精确**（V1 装饰器 + Function 类型） | struct 中用 V1 装饰器装饰 Function 类型变量：变更前编译通过但运行时 crash，变更后编译报错（`The V1 decorator 'xxx' cannot be applied to a Function-type variable 'yyy'`） | grep 装饰器名会匹配所有 V1 用法，需逐一确认属性类型是否 Function；编译报错可直接定位 |
...
| ⚠️**无法 grep 精确匹配**（凡含文本即涉及） | notofonts 三方件小语种字体升级，影响小语种文本渲染 | 跳过 grep，如应用支持小语种直接重点测试文本渲染效果 |
