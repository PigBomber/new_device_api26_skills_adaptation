...
> 以下对照表来自 SDK 的 @deprecated 注解 + 实战迁移验证。升级到 API 26 时，这些是最常见的必改废弃 API。
...
>
> **详细迁移档案**（每个 API 的代码示例+踩坑记录）：见 [references/migration-archive.md](references/migration-archive.md)，含 5 个工程 628 处实战验证。
...
**6. ⚠️ 字段初始化阶段不能用 UIContext（运行时崩溃！）**
...
**7. ⚠️ ContextManager 必须在 loadContent 回调里注入 UIContext**
...
- **[references/migration-archive.md](references/migration-archive.md)** — ⭐ 废弃 API 迁移档案（18 个 API 的替代签名+代码示例+踩坑记录+实战数据，5 个工程 628 处迁移验证）
